 ...
 * for DTMF releated events (AST_FRAME_DTMF_BEGIN and AST_FRAME_DTMF_END).
 * Before 1.4.0 Asterisk only used one frame (AST_FRAME_DTMF) which is now an alias for
 * AST_FRAME_DTMF_END.<br>
 * Many other systems and devices (like mobile phone providers) do not support variable-length DTMF.
 * In these cases you will only see AST_FRAME_DTMF_END frames in Asterisk.<br>
 * When building DTMF aware applications you should not rely on AST_FRAME_DTMF_BEGIN. Generally it
 * is safe to just ignore them and only react on AST_FRAME_DTMF_END frames.<br>
 * To check whether an DtmfEvent represents an AST_FRAME_DTMF_BEGIN or AST_FRAME_DTMF_END frame use
 * the {@link #isBegin()} and {@link #isEnd()} methods.
 * <p/>
...
public class DtmfEvent extends ManagerEvent
{
    ...
    /**
     * Returns whether this event represents an AST_FRAME_DTMF_BEGIN frame.<p>
    ...
    /**
     * Returns whether this event represents an AST_FRAME_DTMF_END frame.<p>
