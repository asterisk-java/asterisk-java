...
public class AgentManager
{
	...
	AsteriskAgentImpl agent = new AsteriskAgentImpl(server,
		event.getName(), "Agent/" + event.getAgent(), AgentState
	...
	if (agent == null)
	{
	    logger.error("Ignored AgentCalledEvent for unknown agent "
		    + event.getAgentCalled());
	    return;
	}
    ...
    {
	logger.info("Set state of agent " + agent.getAgentId() + " to "
		+ newState);
	synchronized (agent)
	{
     ...
     * @param agent
     */
    private void updateRingingAgents(String channelCalling,
	...
	synchronized (ringingAgents)
	{
	    ...
	    {
		updateAgentState(ringingAgents.get(channelCalling),
	...
	if (agent == null)
	{
	    logger.error("Ignored AgentConnectEvent for unknown agent "
		    + event.getChannel());
	    return;
	}
	...
	if (agent == null)
	{
	    logger.error("Ignored AgentCompleteEvent for unknown agent "
		    + event.getChannel());
	    return;
	}
	agent.updateStatus(AgentState.AGENT_IDLE);
