...
public class OriginateAction extends AbstractManagerAction
{
	 ...
	 * or a channel. Hence we support both.
	 * 
	 * Either of the following may be set but NOT BOTH.
	 */
	...
	{
		return "OriginateAction: endPoint/Channel=" + (this._endPoint == null ? this._channel : this._endPoint) + " context=" + this._context //$NON-NLS-1$ //$NON-NLS-2$
				+ " extension=" + this._extension + " callerId=" + this._callerID; //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	public org.asteriskjava.manager.action.ManagerAction getAJAction()
	{
		...
		String channel = (this._channel != null ? this._channel.getChannelName() : this._endPoint
