 ...
 * selected will depend on the actual action that cause the masquerade.
 * 
 * As we proxy all channels a key goal for us is for the existing proxy to end
 * up containing the new channel.
 * 
 * The existing channel will eventually be renamed with a <MASQ> suffix and then
 * go on to become a zombie before it is hungup.
 * 
 * The masquerade event is associated a NewEvent, three renames and one hangup
 * event.
 * 
 * The terminology used by asterisk is quite confusing.
 * 
 * The 'original channel' is the channel created as a result of the action
 * (redirect, park etc). The 'clone' channel is the pre-existing channel that
 * was acted upon.
 * 
 ...
 * like ASYNCGOTO/SIP/101-0000002D:1351178863.121
 * 
 * Masquerade Event: Clone, Original (e.g. SIP/101-0000002D:1351178863.121,
 ...
 * 'clone' is the pre-existing channel).
 * 
 * Rename: Clone to Clone<MASQ> (e.g. rename the pre-existing channel
 * SIP/101-0000002D:1351178863.121 to SIP/101-0000002D:1351178863.121<MASQ> -
 * start shutting down the clone (pre-existing channel). Rename: Original to
 * Clone's initial name Name (e.g rename
 * ASYNCGOTO/SIP/101-0000002D:1351178863.121, SIP/101-0000002D:1351178863.121)
 * Rename:Clone<MASQ> to Clone<Zombie> (e.g. rename
 * SIP/101-0000002D:1351178863.121<MASQ> to
 * SIP/101-0000002D:1351178863.121<ZOMBIE> Hangup: Clone<Zombie> (e.g. hangup
 * SIP/101-0000002D:1351178863.121<ZOMBIE>
 * 
 * Example sequence:
 *
 * RedirectAction: chanel=(proxy=2) SIP/101-0000002F:1351181832.124
 * context=njr-operator exten=njr-musiconhold priority=1 extraContext=null
 * extraChannel=null extraExten=null Dump of LiveChannels, cause:Add: (proxy=3)
 ...
 * ASYNCGOTO/SIP/101-0000002F:1351181837.125
 * 
 * dispatch=NewChannelEvent: channel: (proxy=3)
 * ASYNCGOTO/SIP/101-0000002F:1351181837.125 context=from-internal exten=null
 * state=Up
 * 
 * dispatch=MasqueradeEvent: originalChannel:(proxy=3)
 ...
 * 
 * RenameEvent: existing channel: (proxy=3) SIP/101-0000002F:1351181832.124
 ...
 * SIP/110-0000002E:1351181832.123 ChannelProxy: (proxy=2)
 * ASYNCGOTO/SIP/101-0000002F:1351181837.125 ChannelProxy: (proxy=3)
 * SIP/101-0000002F<MASQ>:1351181832.124
 * 
 * dispatch=RenameEvent: existing channel: (proxy=2)
 * ASYNCGOTO/SIP/101-0000002F:1351181837.125
 ...
 * SIP/101-0000002F<MASQ>:1351181832.124
 * 
 * dispatch=RenameEvent: existing channel: (proxy=3)
 * SIP/101-0000002F<MASQ>:1351181832.124
 * newname=AsyncGoto/SIP/101-0000002f<ZOMBIE>rawExisting:SIP/101-0000002f<MASQ>
 * rawNewname:AsyncGoto/SIP/101-0000002f<ZOMBIE>rawUniquID:1351181832.124
 * 
 * Dump of LiveChannels, cause:RenameEvent: (proxy=3)
 * ASYNCGOTO/SIP/101-0000002F<ZOMBIE>:1351181832.124 ChannelProxy: (proxy=1)
 * SIP/110-0000002E:1351181832.123 ChannelProxy: (proxy=2)
 * SIP/101-0000002F:1351181837.125 ChannelProxy: (proxy=3)
 * ASYNCGOTO/SIP/101-0000002F<ZOMBIE>:1351181832.124
 * 
 * dispatch=BridgeEvent: channel1:(proxy=1) SIP/110-0000002E:1351181832.123
 * channel2:(proxy=3) ASYNCGOTO/SIP/101-0000002F<ZOMBIE>:1351181832.124
 * bridgeState:Unlink
 * 
 ...
 * ASYNCGOTO/SIP/101-0000002F<ZOMBIE>:1351181832.124 ChannelProxy: (proxy=1)
 * SIP/110-0000002E:1351181832.123 ChannelProxy: (proxy=2)
 * SIP/101-0000002F:1351181837.125
 * 
 * dispatch=HangupEvent: channel:(proxy=1) SIP/110-0000002E:1351181832.123
 * cause:Normal Clearing Dump of LiveChannels, cause:Removing: (proxy=1)
 * SIP/110-0000002E:1351181832.123 ChannelProxy: (proxy=2)
 * SIP/101-0000002F:1351181837.125
 * 
 * The end result is that the original (new) channel ends up with the clones
 * (pre-existing) channel name.
 * 
 * The pre-existing proxy ends up holding the original channel with the
 * pre-existing channel's initial name.
 * 
 * The masquerade event then is about ripping the guts out of the clone
 * (pre-existing channel) and swapping with the original (new) channel. At the
 * end of the swap the original (new) channel is left active (as it has the
 * clones guts) and the clone (pre-existing) is inactive so we can hangup the
 * clone as it is no longer needed.
 * 
 * 
 * 
 * @author bsutton
...
public class MasqueradeEvent extends ManagerEvent
{
	...
	private static final Logger logger = Logger.getLogger(MasqueradeEvent.class);
	...
	public MasqueradeEvent(final org.asteriskjava.manager.event.MasqueradeEvent event) throws InvalidChannelName
	{
		...
		this._original = pbx.registerChannel(event.getOriginal(), ChannelImpl.UNKNOWN_UNIQUE_ID);
		this._clone = pbx.registerChannel(event.getClone(), ChannelImpl.UNKNOWN_UNIQUE_ID);
