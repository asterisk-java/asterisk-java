 ...
 * 
 * It should be noted that the Peer must keep track of all calls regardless of
 * whether they are associated with NJR. The reason for this is that a Peer
 * needs to track the status of the associated endpoint so that it can show if a
 * handset is on a call. In this way the operator can tell if a person is busy
 * even before they call them.
 * 
 * Essentially all this calls does is track the set of channels associated with
 * a call.
 * 
 * @author bsutton
 * 
...
{
	private static final Logger logger = Logger.getLogger(CallTracker.class);
		...
		synchronized (this._associatedChannels)
		{
			...
			if (index != -1)
			{
				if (logger.isDebugEnabled())
					logger.debug("CallTracker removing channel: " + this.toString() + " " + channel.getExtendedChannelName()); //$NON-NLS-1$ //$NON-NLS-2$
