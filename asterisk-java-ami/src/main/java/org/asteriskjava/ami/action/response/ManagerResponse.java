...
/**
 * Represents a response received from the Asterisk server as the result of a previously sent {@link ManagerAction}.<p>
 * The response can be linked with the action that caused it by looking the {@code ActionID} attribute that will match
 * the action id of the corresponding action.
 *
...
public class ManagerResponse implements Serializable {
    ...
    /**
     * Asterisk argument: {@code ActionID}.<p>
     * Returns the user provided action id of the ManagerAction that caused this response.
     * If the application did not set an {@code ActionID} this method returns {@code null}.
     *
     ...
     * The keys are all <b>lowercase</b>.
     *
     ...
     * This is particularly important when a response contains special attributes that are dependent on the action that
     * has been sent.<p>
     * An example of this is the response to the {@code GetVarAction}.
     * It contains the value of the channel variable as an attribute stored under the key of the variable name.
     * <p>
     * Example:
     * <pre>
     ...
     * </pre>
     * <p>
     * As all attributes are internally stored in <b>lowercase</b> the key is automatically converted to lower case
     * before lookup.
     *
     * @param key the key to lookup
     * @return the value of the attribute stored under this key or {@code null} if there is no such attribute
     */
    public String getAttribute(String key) {
        return (String) attributes.get(key.toLowerCase(ENGLISH));
    }
