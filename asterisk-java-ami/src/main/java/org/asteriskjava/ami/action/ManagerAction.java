...
/**
 * Interface that all actions that can be sent to the Asterisk server must implement.<p>
...
public interface ManagerAction extends Serializable {
    ...
    /**
     * Sets Asterisk argument: {@code ActionID}.<p>
     * If the action id is set and sent to the Asterisk any response returned by the Asterisk will include the same id.
     ...
     * and events. Though the internal action is never exposed to the application code.
     * So if you want to handle responses or response events on your own, your application must set a unique
     * {@code ActionID} using this method otherwise the {@code ActionID} of the responses and response event objects
